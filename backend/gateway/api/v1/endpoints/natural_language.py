...
async def sentiment_analysis_prediction(
    ...
    return create_response(message="Prediction got succesfully", data=prediction) # type: ignore
...
#     """
#     delay_elapsed = datetime.now(timezone.utc)+ timedelta(seconds=seconds)
...
async def get_result_from_batch_task(task_id: str) -> IPostResponseBase:
    ...
    """
    async_result = celery.AsyncResult(task_id) # type: ignore
